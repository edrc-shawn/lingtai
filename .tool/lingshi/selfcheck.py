...
检查项：
1. MCP配置 → mcp.json 中 lingshi 配置是否存在
...
用法：
    ...
    python 灵台/.tool/lingshi/selfcheck.py --fix     # 检查并修复
"""
...
# 路径常量
VAULT = Path(r"os.environ.get("LINGTAI_VAULT", "")")
WORKBUDDY = Path(os.path.expanduser("~")) / ".workbuddy"
MCP_CONFIG = WORKBUDDY / "mcp.json"
MCP_SERVER = VAULT / ".tool" / "lingshi" / "mcp_server.py"
...
def _fix_mcp_config_disabled() -> Tuple[bool, str]:
    ...
    try:
        ...
        config["mcpServers"]["lingshi"]["enabled"] = True
        config["mcpServers"]["lingshi"]["disabled"] = False
        
        with open(MCP_CONFIG, "w", encoding="utf-8") as f:
            json.dump(config, f, indent=2, ensure_ascii=False)
        
        return True, "已启用 lingshi 配置"
    except Exception as e:
        return False, str(e)
    ...
    try:
        ...
        if not lingshi:
            ...
            def fix_add_config():
                try:
                    if "mcpServers" not in config:
                        config["mcpServers"] = {}
                    config["mcpServers"]["lingshi"] = DEFAULT_MCP_CONFIG["mcpServers"]["lingshi"]
                    with open(MCP_CONFIG, "w", encoding="utf-8") as f:
                        json.dump(config, f, indent=2, ensure_ascii=False)
                    return True, "已添加 lingshi 配置"
                except Exception as e:
                    return False, str(e)
            ...
            return CheckResult(
                "MCP配置",
                "⚠️",
                "mcp.json 中未找到 lingshi 配置",
                "添加 lingshi MCP服务器配置",
                fixable=True,
                fix_func=fix_add_config
            )
        
        if not lingshi.get("enabled", True):
            return CheckResult(
                "MCP配置",
                "⚠️",
                "lingshi 配置已禁用",
        ...
        return CheckResult(
            "MCP配置",
            "✅",
            f"lingshi 配置正常 (command: {lingshi.get('command', 'N/A')})"
        )
...
def check_mcp_server() -> CheckResult:
    ...
    try:
        ...
        if "class LingtaiMCPServer" not in content:
            return CheckResult(
                "MCP服务器",
                "⚠️",
                "mcp_server.py 中未找到 LingtaiMCPServer 类",
                "文件可能已损坏，重新安装灵识模块"
            )
...
def check_llm_config() -> CheckResult:
    ...
    try:
        ...
        has_key = any(m.get("apiKey") for m in models)
        if not has_key:
            return CheckResult(
                "LLM配置",
                "⚠️",
