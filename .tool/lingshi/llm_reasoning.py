...
使用方法：
    from .tool.lingshi.llm_reasoning import LLMReasoning
...
class LLMReasoning:
    ...
    def __init__(self, config_path: str = None):
        ...
        Args:
            config_path: 配置文件路径（models.json）
        """
        if config_path is None:
            local_config = os.path.join(os.path.dirname(__file__), "..", "..", "存档", "API", "models.json")
            if os.path.exists(local_config):
                config_path = local_config
            else:
                config_path = os.path.join(os.path.expanduser("~"), ".workbuddy", "models.json")
    ...
    def _call_llm(self, prompt: str, max_tokens: int = 2048) -> str:
        ...
        try:
            ...
            response = requests.post(
                self.config.get("endpoint", f"{self.config['url']}/chat/completions"),
                headers=headers,
                json=data,
                timeout=30
            )
